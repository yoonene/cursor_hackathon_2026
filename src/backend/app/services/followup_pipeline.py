...
def prepare_follow_up_tooling(
    ...
    *,
    ...
    traces: list[AgentTraceStep] = []
    _ensure_compat_intake_coherence(state, board, traces)
    if structured_partner is not None:
        _prime_state_for_partner_request(state, board, traces)

    msg = latest_message.strip()
    effective_structured = structured_partner

    partner_birth_resolved = (
        effective_structured.birth_date if effective_structured is not None else extract_first_birth_date(msg)
    )

    reading = getattr(board, "active_reading", None)
    board_sk = getattr(reading, "semantic_key", None) if reading is not None else None
    if not isinstance(board_sk, str):
        board_sk = "compatibility:user:partner_pending"

    if (
        _user_requests_partner_ui_refresh(msg)
        and _prior_active_template(board) == "compatibility_pending"
        and state.current_stage == "collecting_compatibility_info"
        and partner_birth_resolved is None
    ):
        supplemental = json.dumps(
            {
                ...
                "re_prompt_ui": True,
                "instructions": (
                    "The client asked to reopen the partner intake UI while awaiting a counterpart "
                    "birth date. Keep context and politely invite filling the intake again."
                ),
            },
            ensure_ascii=False,
        )
    ...
    compat_handled = _compat_collection_turn(
        state,
        msg,
        board,
        traces,
        structured_partner=effective_structured,
    )
    if compat_handled is not None:
        return compat_handled

    prev = _prior_active_template(board)
    intent = classify_follow_up_intent(settings, msg)
    traces.append(
        AgentTraceStep(
            step="routing",
    ...
    if intent.route == "domain_fortune":
        fd = intent.fortune_domain or "overall"
        fp = intent.fortune_period or "this_week"
        res = analyze_domain_fortune(state.user_saju, fd, fp, date.today())
        rd, supplemental = attach_domain_fortune(board, res)
        if board.active_reading is not None:
            state.readings_by_semantic_key[res.semantic_key] = board.active_reading
        traces.append(
            AgentTraceStep(
                step="tool_call",
                label=f"analyze_domain_fortune {res.semantic_key}",
    ...
    if intent.route == "favorable_timing":
        td = intent.timing_domain or "general"
        at = intent.action_type or "other"
        res = analyze_favorable_timing(state.user_saju, td, at, date.today())
        ...
        traces.append(
            AgentTraceStep(
                step="tool_call",
                label=f"analyze_favorable_timing {res.semantic_key}",
    ...
    partner_dt = intent.partner_birth_date or extract_first_birth_date(msg)
    if partner_dt is None:
        state.current_stage = "collecting_compatibility_info"
        state.pending_tool_call = PendingToolCall(
            tool_name="analyze_compatibility",
            semantic_key="compatibility:user:partner_pending",
            missing_fields=["counterpart_birth_date"],
            collected_inputs=( {"partner_name_hint": intent.partner_name} if intent.partner_name else {}),
        )
        rd, _ = attach_compatibility_pending(board, state.user_profile.display_name)
        if board.active_reading is not None:
            state.readings_by_semantic_key[state.pending_tool_call.semantic_key] = board.active_reading
        traces.append(
            AgentTraceStep(
                step="tool_call",
        ...
        supplemental = json.dumps(
            {
                "tool": "compatibility_pending",
                "semantic_key": state.pending_tool_call.semantic_key,
                "instructions": (
    ...
    partner_nm = intent.partner_name
    partner = _partner_profile(state.session_id, partner_nm, partner_dt)
    other_saju = analyze_base_saju(partner)
    compat = analyze_compatibility(state.user_saju, other_saju)
    _remember_related(state, partner, other_saju)
    ...
    traces.append(
        AgentTraceStep(
            step="tool_call",
            label=f"analyze_compatibility immediate ({compat.semantic_key})",
            tool_name="analyze_compatibility",
            status="completed",
        ),
    )
